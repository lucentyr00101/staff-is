...
<div>
	...
	<table class="table table-striped table-hover" id="data-table">
		...
		<tbody>
			@foreach($invited_tasks as $task)
			@if($task->is_read == 0 && $task->status == 'invited')
			<tr class='clickable-row' data-href="{{ URL::to('tasks/' . $task->workforce->id . '&e=' . $employee->id) }}">
				...
				<td><a href="{{ URL::to('tasks/' . $task->workforce->id . '&e=' . $employee->id) }}" class='btn btn-info'>View</a></td>
			</tr>
			@else
			<tr class='clickable-row' data-href="{{ URL::to('tasks/' . $task->workforce->id . '&e=' . $employee->id) }}">
				<td></td>
				...
				<td>{{$task->workforce->work_type}}</td>
				<td>{{ $task->workforce->extra_requirements }}</td>
				<td><a href="{{ url('/employee/accept/task=' . $task->workforce_id . '&employee=' . $task->employee_id) }}" class='btn btn-info'>View</a></td>
			</tr>
			@endif
			@endforeach
		</tbody>
